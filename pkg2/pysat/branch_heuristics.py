...
from pkg2.pysat.solver import Solver
from pkg2.utils.constants import UNASSIGN, TRUE, FALSE, TRAIL_EMPTY
...
#abhishek's edits
class ForcedBranch(Solver):
    """
    take decisions based on the values in a specified file
    """
    def pick_branching_variable(self):
        
        #while abs(self.trail[self.index]) not in self.all_unassigned_vars():
        print("Index:{}".format(self.index))
        if(self.index >= len(self.trail)):
            return TRAIL_EMPTY, UNASSIGN
        
        return abs(self.trail[self.index]), TRUE if self.trail[self.index] > 0 else FALSE
