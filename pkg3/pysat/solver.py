...
from pkg3.utils.constants import TRUE, FALSE, UNASSIGN
from pkg3.utils.exceptions import FileFormatError
from pkg3.utils.logger import set_logger
...
class Solver:
    ...
    def __init__(self, filename, decisions):
        logger.info('========= create pysat from %s =========', filename)
        self.filename = filename
        self.cnf, self.vars = Solver.read_file(filename, decisions)
        self.learnts = set()
    ...
    @staticmethod
    def read_file(filename, decisions):
        """
        ...
        decisions = decisions.split('_')
        for bvar in decisions:
            clauses.add(frozenset({int(bvar)}))	
